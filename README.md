...
# Appendix: MVP (Minimum Viable Product)

## Цель MVP

Создать минимально работающий прототип HiDocu, который реализует **ядро концепции** — Context Management System — без
внешних интеграций, облачных сервисов и автоматизации. Все действия, для которых существует ручная альтернатива,
выполняются вручную.
...
- Вложенные папки произвольной глубины.
- Системные папки: **All Documents** (виртуальная, все документы отсортированные по дате создания desc), **Trash** (
  только `local_trash` — физическое перемещение файлов в локальную корзину, без git-составляющей).
- У папки может быть задан **Transcription Context** (статический, plain text) — хранится в `config.yaml` папки.
- У папки может быть задан **Categorization Context** (статический, plain text) — хранится в `config.yaml` папки.
- Настройка `prefer_summary` (использовать Summary или Body при формировании контекста) — на уровне папки.
- Настройка `minimize_before_llm` — присутствует в модели данных, но **не выполняет никаких действий** (контент не
  минимизируется).
- Настройки наследуются от родительской папки, могут быть переопределены или дополнены на уровне вложенной папки.
- **Нет** системной папки "Uncategorized" (т.к. нет автоматической сортировки — все документы создаются пользователем
  вручную в нужной папке).
- **Нет** Sorting Rules.
- **Нет** LLM-Judge для категоризации.

### 2. Documents (Документы)

- Создание, переименование, удаление, перемещение документов между папками.
- Документ состоит из:
  - **Body** (`body.md`) — Markdown-текст, отображается и редактируется как plain text (as-is, без рендеринга
    Markdown).
  - **Summary** (`summary.md`) — Markdown-текст, создаётся и редактируется вручную, отображается как plain text.
  - **Metadata** (`metadata.yaml`) — название, дата создания/обновления, `document_type`, `prefer_summary`,
    `minimize_before_llm`.
- Физическое хранение: папка `.document` с файлами `body.md`, `summary.md`, `metadata.yaml`, папка `sources/`.
- Все документы создаются сразу в статусе **approved** (нет Approval Workflow).
- `git_committed` всегда `0` (нет работы с git).
- Настройка `minimize_before_llm` — присутствует, но **не выполняет никаких действий**.
- Удаление документа: перемещение `.document`-папки в локальную корзину (`~/Library/Application Support/HiDocu/trash/<document_id>/`),
  запись в `deletion_log` c `restore_strategy = 'local_trash'`. Автоочистка через 30 дней.
- Восстановление из Trash: перемещение из локальной корзины обратно + реиндексация в БД.
- **Нет** поиска документов (FTS не реализуется).
- **Нет** реиндексации БД из файловой системы.

### 3. Sources (Источники)

В MVP поддерживается **только один тип источника**: Recording.

- Источник хранится в папке `.source` внутри `.document`.
- Каждый источник имеет `metadata.yaml` с метаданными.
- **Нет** Calendar Event, PDF, Image, JSON, Dynamic Sources.
- Источники прикрепляются к документу (один документ может иметь несколько Sources).
- Пользователь может вручную добавлять и удалять источники у документа.

### 4. Recordings (Аудиозаписи)

- **Импорт** поддерживается двумя способами:
  - **С устройств HiDock** (H1, H1E, P1, P1 Mini): автоматическое обнаружение при подключении, импорт файлов.
  - **Вручную**: drag-and-drop аудиофайлов в интерфейс HiDocu.
- Импорт выполняет **только шаги 1 и 2** типичного workflow:
  1. Создание Source для аудиофайла (копирование файла, создание `.source`-папки).
  2. Извлечение метаданных **только из имени файла и timestamp файловой системы** (дата, имя). Никакие
     аудиобиблиотеки не подключаются — duration, sample_rate, channels, audio_format **не извлекаются** (остаются
     NULL в БД).
- Импорт **не** создаёт Document автоматически. Пользователь вручную создаёт документ и прикрепляет к нему
  импортированные Sources.
- Аудиофайлы хранятся **только в локальной файловой системе** (в папке `.source` или в указанной пользователем
  директории для бинарных файлов).
- **Нет** аудиопроигрывателя.
- **Нет** автоматической транскрипции.
- **Нет** автоматического создания документов на основании Recording.
- **Нет** автоматической привязки к календарным событиям.

### 5. Transcripts (Стенограммы)

- Стенограммы создаются **вручную**: пользователь вставляет текст (copy-paste) стенограммы, полученной из внешнего
  сервиса (например, из web-интерфейса Whisper, AssemblyAI, или любого другого).
- Один Recording может иметь несколько вариантов стенограмм.
- Пользователь вручную помечает один из вариантов как **Primary**.
- При выборе Primary стенограммы её содержимое копируется в Body документа (с предупреждением о потере ручных правок
  Body, если Body был отредактирован).
- **Нет** спикеров, таймстемпов, аудиоплеера.
- **Нет** LLM-Judge для выбора лучшей стенограммы.
- `primary_selected_by` всегда `'user'`.
- `is_draft` всегда `0` (нет черновых стенограмм для сортировки).

### 6. Context (Формирование контекста)

- Контекст формируется из документов папки (включая вложенные папки): конкатенация Summary (или Body, в зависимости от
  настроек документа/папки) всех документов.
- Если у документа нет Summary или настройка `prefer_summary = false` — используется Body.
- Контекст можно **скопировать в буфер обмена** в формате plain Markdown.
- Подсчёт токенов: **generic-токенизатор**, считающий количество байт (не слов, не BPE-токенов).
- Отображается приблизительное количество «токенов» (байт) для папки и для отдельного документа.
- **Нет** выбора целевой модели, нет знаний о лимитах контекстных окон моделей, нет цветовой индикации
  (зелёный/жёлтый/красный).
- **Нет** формата `xml` или `plain` при копировании — только `markdown` (as-is).
- `minimize_before_llm` не выполняет никаких действий.

### 7. Database (SQLite)

Реализуются следующие таблицы (упрощённые, без полей, связанных с отсутствующими функциями):

| Таблица                  | Статус в MVP                                                                      |
|--------------------------|-----------------------------------------------------------------------------------|
| `folders`                | ✅ Реализуется. Без `is_repo_root`, `binary_storage_strategy`, `external_storage_path`, `transcription_model`, `transcription_variants`, `transcription_judge_model`. |
| `documents`              | ✅ Реализуется. `approval_status` всегда `'approved'`, `git_committed` всегда `0`, `last_commit_sha` всегда NULL. |
| `sources`                | ✅ Реализуется. `source_type` только `'recording'`. Поля динамических источников (`is_dynamic`, `dynamic_trigger`, `dynamic_command`, `cache_ttl`, `last_fetched_at`) всегда NULL/0. |
| `recordings`             | ✅ Реализуется. `duration_ms`, `audio_format`, `sample_rate`, `channels` — всегда NULL (нет аудиобиблиотек). `import_source_id` — заполняется для HiDock, NULL для ручного импорта. |
| `transcripts`            | ✅ Реализуется. `model` = `'manual'`. `status` сразу `'completed'`. `is_draft` всегда `0`. `primary_selected_by` всегда `'user'`. |
| `audio_import_sources`   | ✅ Реализуется. `source_type` только `'hidock'` и `'manual'`. |
| `deletion_log`           | ✅ Реализуется. `restore_strategy` всегда `'local_trash'`. |
| `document_token_cache`   | ✅ Реализуется. `tokenizer` всегда `'bytes'`. |
| `folder_token_cache`     | ✅ Реализуется. `tokenizer` всегда `'bytes'`. |
| `jobs`                   | ❌ Не реализуется (нет фоновых задач, требующих очереди). |
| `sorting_rules`          | ❌ Не реализуется. |
| `calendars`              | ❌ Не реализуется. |
| `calendar_events`        | ❌ Не реализуется. |
| `calendar_event_participants` | ❌ Не реализуется. |
| `api_keys`               | ❌ Не реализуется. |
| `git_sync_state`         | ❌ Не реализуется. |
| `conflicts`              | ❌ Не реализуется. |
| `documents_fts`          | ❌ Не реализуется (нет поиска). |
| `schema_version`         | ✅ Реализуется. |

### 8. Settings (Настройки)

Минимальный `settings.yaml`:
...
Из глобальных настроек реализуются:

| Раздел          | Статус                                                                 |
|-----------------|------------------------------------------------------------------------|
| General         | ✅ `language`, `data_directory` только.                                |
| API Keys        | ❌ Не реализуется.                                                     |
| Local Models    | ❌ Не реализуется.                                                     |
| Default Models  | ❌ Не реализуется.                                                     |
| Audio Import    | ✅ Только `auto_detect_new_devices`, `import_file_formats`, `min_recording_duration`, `devices` (HiDock). |
| Calendars       | ❌ Не реализуется.                                                     |
| Git             | ❌ Не реализуется.                                                     |
| Context & Tokens| ✅ Только `default_context_source`, `copy_context_format` (только `markdown`). |
| Built-in Chat   | ❌ Не реализуется.                                                     |
| Privacy         | ❌ Не реализуется (нет сетевых запросов).                               |
| Notifications   | ✅ Только `import_complete`.                                           |

### 9. UI (Интерфейс)

- Трёхколоночный layout: Sidebar (папки) → Document List → Main View (документ).
- Sidebar: дерево папок, навигация.
- Document List: список документов в выбранной папке (название, дата, превью).
- Main View:
  - Отображение Body и Summary как plain text (без рендеринга Markdown).
  - Редактирование Body и Summary в текстовых полях.
  - Список Sources (аудиофайлов) с базовыми метаданными (имя файла, дата из файловой системы).
  - Список Transcripts для каждого Source с возможностью: добавить вариант (вставить текст), пометить как Primary,
    удалить.
  - Отображение количества байт (как «токенов») для Body и Summary.
- All Documents: виртуальная папка, показывающая все документы отсортированные по дате создания (desc).
- Trash: список удалённых документов с возможностью восстановления.
- Settings: минимальный экран настроек (data directory, HiDock devices).
- Drag-and-drop аудиофайлов для ручного импорта.
- Формирование и копирование контекста из папки (кнопка «Copy Context»).
- Отображение общего количества байт контекста для папки.
- **Нет** аудиопроигрывателя.
- **Нет** поиска.
- **Нет** Conflict Resolution UI.
- **Нет** Approval UI (все документы сразу approved).
...
|--------------------------------------|--------------------------------------------------------------------|
| Работа с LLM (транскрипция, Summary, Judge, категоризация) | Нет внешних интеграций. Всё вручную. |
| Работа с git                         | Нет версионирования. Только локальная файловая система.            |
| Работа с календарями                 | Нет внешних интеграций.                                            |
| Работа с облачными хранилищами       | Нет внешних интеграций.                                            |
| API Keys                             | Нет облачных сервисов.                                             |
| Импорт из iCloud, Dropbox, folder watch | Нет внешних интеграций. Только HiDock + ручной импорт.          |
| Автоматическое создание документов   | Нет автоматизации. Пользователь создаёт документы вручную.        |
| Автоматическая сортировка (Sorting Rules, LLM-Judge) | Нет автоматизации. Пользователь сортирует вручную. |
| Approval Workflow                    | Нет автоматизации → нет необходимости в проверке.                  |
| Папка "Uncategorized"                | Нет автоматической сортировки → нет uncategorized документов.      |
| PDF, Image, JSON sources             | Нет обработки этих форматов.                                       |
| Dynamic Sources                      | Нет внешних интеграций и shell-команд.                             |
| Рендеринг Markdown                   | Отображение as-is. Рендеринг будет добавлен позже.                 |
| Аудиопроигрыватель                   | Нет аудиобиблиотек.                                                |
| Анализ аудиофайлов (duration, format, sample rate) | Нет аудиобиблиотек. Метаданные только из имени/timestamp файла. |
| Определение спикеров, таймстемпы     | Нет обработки аудио.                                               |
| Специальные токенизаторы             | Только подсчёт байт.                                               |
| Знания о моделях и лимитах контекстного окна | Нет конфигураций моделей.                                  |
| `minimize_before_llm` (реальная минимизация) | Флаг присутствует, но ничего не делает.                     |
| Встроенный чат                       | Нет API Keys → нет чата. Контекст через copy-paste.               |
| Conflict Resolution UI               | Нет git → нет конфликтов.                                         |
| Хранение в Git LFS                   | Нет git.                                                           |
| Полнотекстовый поиск (FTS5)          | Не реализуется.                                                    |
| Реиндексация БД из файловой системы  | Не реализуется.                                                    |
| Очередь фоновых задач (jobs)         | Нет фоновых процессов.                                             |
| Динамический Transcription/Categorization Context | Только статический текст.                              |
| OAuth / CalDAV / EventKit            | Нет внешних интеграций.                                            |

---

## Пользовательские сценарии MVP

### Сценарий 1: Создание структуры папок

1. Пользователь открывает HiDocu.
2. Создаёт папку "Company X".
3. Внутри создаёт "1:1s" → "1:1s with John".
4. Задаёт Categorization Context для "1:1s with John": `Keywords: John, 1:1, Meeting`.
5. Задаёт Transcription Context для "Company X": `Domain: KPI, OKR, API. Employees: John Doe (CEO), Sarah Smith (CTO)`.

### Сценарий 2: Ручное создание документа

1. Пользователь открывает папку "1:1s with John".
2. Создаёт новый документ "1:1 with John — 2026-02-03".
3. Вписывает в Body стенограмму встречи (copy-paste из внешнего сервиса).
4. Вписывает в Summary краткое резюме.
5. Документ сохраняется на диск.

### Сценарий 3: Импорт аудио с HiDock

1. Пользователь подключает HiDock H1.
2. HiDocu обнаруживает устройство, предлагает настроить импорт.
3. Пользователь включает auto-import.
4. Аудиофайлы копируются с устройства, создаются Sources с метаданными (имя файла, timestamp).
5. Пользователь вручную создаёт документ, прикрепляет к нему импортированные Sources.
6. Пользователь вручную транскрибирует аудио во внешнем сервисе и вставляет стенограмму как Transcript.
7. Помечает стенограмму как Primary — текст копируется в Body документа.

### Сценарий 4: Работа с несколькими вариантами стенограмм

1. У документа есть Source (аудиозапись).
2. Пользователь вставляет 3 варианта стенограммы (из разных сервисов или с разными настройками).
3. Пользователь читает все 3 варианта, выбирает лучший и помечает его как Primary.
4. Body документа обновляется выбранным транскриптом.

### Сценарий 5: Формирование контекста и использование с LLM

1. Пользователь открывает папку "1:1s with John" (в ней 5 документов-стенограмм).
2. Видит общее количество байт контекста.
3. Нажимает «Copy Context».
4. Контекст (конкатенация Summary всех документов) копируется в буфер обмена.
5. Пользователь вставляет контекст в ChatGPT/Claude/Gemini и задаёт вопрос.

### Сценарий 6: Удаление и восстановление документа

1. Пользователь удаляет документ из папки.
2. Документ появляется в Trash.
3. Пользователь может восстановить документ из Trash (в течение 30 дней).
4. После 30 дней документ автоматически удаляется из Trash.
