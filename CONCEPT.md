...
Концепт HiDocu — это Artificial Memory - дополнительная искуственная память для человека, куда он может offload'ить свои
контексты: знания, встречи, заметки, идеи, планы и т.п., а затем использовать контексты из этой памяти для общения с LLM
и получения инсайтов, принятия решения о следующих шагах, ответов на вопросы, генерации идей и т.п.
...
HiDocu состоит из таких модулей и подмодулей:

- Context Management System (Context) — система управления контекстом для LLM, которая позволяет пользователю создавать
  контексты из своих документов и использовать эти контексты для общения с любой LLM. С такими подсистамами, как:
    - Transcription System — подсистема для транскрипции и анализа, которая позволяет
      пользователю транскрибировать аудиозаписи с помощью локальных или облачных моделей, а также анализировать эти
      транскрипты с помощью LLM для получения инсайтов, резюме, извлечения задач и т.п.
    - Chat System — подсистема общения с LLM, которая позволяет пользователю общаться с любой LLM,
      используя
      контексты.
- Recording Management System (RMS) — система управления аудиозаписями, которая позволяет пользователю импортировать
  аудиозаписи встреч, управлять устройствами аудиозаписи и облачными сервисами откуда можно импортировать аудиозаписи, а
  также автоматически бэкапить/синхронизировать эти аудиозаписи в облако по выбору пользователя (например, iCloud,
  Dropbox и т.п.)
- Calendar(CES) — система управления календарём и событиями, которая позволяет пользователю интегрировать
  свои календарные аккаунты (например, Google Calendar, Microsoft Outlook и т.п.), автоматически импортировать события из
  календаря и использовать эти события для автоматической генерации документов в HiDocu на основании этих событий и
  аудиозаписей встреч, связанных с этими событиями.

Модули общаются между собой используя event-driven подход.
Модули хранят свои данные в файловой системе и в БД отдельно друг от друга, но при этом могут ссылаться друг на друга
через эти хранилища. Например, документ в CMS может ссылаться на аудиофайл в RMS как на источник для этого документа,
а аудиофайл в RMS может ссылаться на документ в CMS как на документ, который был сгенерирован на основании этого
аудиофайла.

В файловой системе модули имеют свои папки для хранения своих данных:
...
UI/UX приложения соответствует современным стандартам macOS и Apple Human Interface Guidelines. Основные элементы
интерфейса:
...
- **Трех колоночный Layout для работы с папками и документами:**
    ...
    - **Document List View:** Список документов внутри выбранной папки с краткой информацией (название, дата, статус
      транскрипции, связанное событие календаря и т.п.)
    - **Main View:** Содержимое выбранного документа, включая метаданные, стенограммы, summary, связанные события
      календаря, sources (аудиофайлы) и т.п.
